export default function BottomNav({ activeTab, setActiveTab }) {
    const tabs = [
        ...
        { id: "post", icon: "fa-plus", label: "HANTAR", isCenter: true },
        { id: "chat", icon: "fa-comment-dots", label: "MESEJ", hasNotif: true },
    ...
    return (
        <>
            ...
            <nav className="fixed bottom-0 left-0 right-0 bg-white/85 backdrop-blur-xl border-t border-white/50 flex justify-around items-center py-3.5 px-6 z-50 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] md:hidden">
                {tabs.map((tab) => {
                    ...
                    if (tab.isCenter) {
                        return (
                            ...
                            >
                                ...
                                >
                                    <i className={`fa-solid ${isActive ? "fa-wand-magic-sparkles" : tab.icon} text-xl`}></i>
                                </div>
                            </button>
                        )
                    }
                    ...
                    return (
                        <button
                            key={tab.id}
                            onClick={() => setActiveTab(tab.id)}
                            className={`flex flex-col items-center gap-1.5 transition-all active:scale-90 relative ${isActive ? "text-bantu-orange" : "text-gray-400 hover:text-bantu-dark"
                                }`}
                        >
                            <i className={`fa-solid ${tab.icon} text-xl ${isActive ? "drop-shadow-sm" : ""}`}></i>
                            {tab.hasNotif && (
                        ...
                        return (
                            ...
                            >
                                <div className={`w-9 h-9 rounded-lg flex items-center justify-center transition-colors ${isActive
                                    ...
                                    <i className={`fa-solid ${tab.isCenter && isActive ? "fa-wand-magic-sparkles" : tab.icon} text-sm`}></i>
                                </div>
                ...
                <div className="p-4 border-t border-gray-100">
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer">
                        <img
                            ...
                            className="w-10 h-10 bg-white rounded-full border-2 border-bantu-orange shadow-sm"
                            alt="Shahmizan"
                        />
                        <div className="flex-1 min-w-0">
                            <p className="font-bold text-sm text-gray-800 truncate">Shahmizan</p>
                            <p className="text-[10px] text-gray-400 font-medium">Lvl 5 · 1,250 BP</p>
                        </div>
